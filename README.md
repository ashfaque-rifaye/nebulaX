...
NebulaX is a living team-intelligence fabric. You define an **Intelligence Mission** in plain language; an autonomous agent swarm senses the web, extracts the real data behind each finding, cross-checks it across sources, builds a side-by-side comparison, surfaces any conflicts for you to resolve, and then turns the analysis into a prototype and a ranked build plan routed into the tools your team already uses. Every finding shows its real metrics, a **Verified / Needs review** badge and its sources — no opaque confidence scores.

## Architecture

- **Frontend** — Vite + React 19 + TypeScript + Tailwind v4 + Motion (`src/App.tsx`, `src/components/`). A unified Swarm Workspace: six lenses over one fabric (Evidence Board, Flow Map, **Build Studio**, Temporal Vista, Triage, Replay), a docked Inspector panel (finding detail / agent console / grounded chat / custom agents / mission pulse), a live agent-activity ticker with an expandable Brief & Actions pane, and a first-run spotlight tour. Findings carry real data (metrics / N-way comparison tables), a binary verified badge and a source count.
