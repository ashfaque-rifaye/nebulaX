...
const card = (isDark: boolean) =>
  isDark
    ...
    : "bg-white border-violet-500/10 shadow-lg shadow-violet-500/5";
const title = (isDark: boolean) => (isDark ? "text-white" : "text-slate-900");
const muted = (isDark: boolean) => (isDark ? "text-white/55" : "text-slate-500");
const eyebrow = (isDark: boolean) =>
  `text-[11px] font-bold tracking-[0.2em] uppercase ${isDark ? "text-luna-lavender" : "text-violet-700"}`;
const ctaPill =
  "bg-luna-gradient cta-luna text-white text-sm font-bold rounded-full flex items-center gap-2";
...
export const HowItWorksPage: React.FC<PageProps> = ({ isDark, onLaunch }) => (
  <main className="flex-1 overflow-y-auto">
    ...
    <header className="text-center flex flex-col items-center gap-3 scroll-reveal">
      <span className={eyebrow(isDark)}>How it works</span>
      <h2 className={`text-3xl md:text-5xl font-extrabold tracking-tight ${title(isDark)}`}>From a sentence to a <span className="text-gradient">verified, actionable brief</span></h2>
    ...
    <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
      {PIPELINE_STEPS.map((s, i) => (
        <div key={s.n} className={`group relative rounded-[2rem] border p-5 flex flex-col gap-2 transition-all duration-500 scroll-reveal bg-gradient-to-br from-violet-500/[0.07] to-transparent ${card(isDark)}`}>
          <div className="flex items-center justify-between">
            <span className={`w-10 h-10 rounded-2xl flex items-center justify-center glass border group-hover:scale-110 transition-transform duration-500 ${isDark ? "border-white/10" : "border-violet-500/15"} ${s.color}`}><Icon name={s.icon} className="w-5 h-5" /></span>
    ...
    <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {[
        ...
        <div key={i} className={`group rounded-[2rem] border p-6 flex flex-col gap-2 transition-all duration-500 scroll-reveal ${card(isDark)}`}>
          <div className={`w-12 h-12 rounded-2xl glass border flex items-center justify-center group-hover:scale-110 transition-transform duration-500 ${isDark ? "border-white/10" : "border-violet-500/15"}`}>{b.icon}</div>
    ...
    <div className="flex justify-center scroll-reveal">
      <button onClick={() => onLaunch()} className={`${ctaPill} px-7 py-3.5`}>
...
export const AgentsPage: React.FC<PageProps> = ({ isDark, onLaunch }) => (
  <main className="flex-1 overflow-y-auto">
    <div className="max-w-6xl mx-auto w-full p-4 md:p-8 flex flex-col gap-8">
    <header className="text-center flex flex-col items-center gap-3 scroll-reveal">
      <span className={eyebrow(isDark)}>Meet the swarm</span>
      <h2 className={`text-3xl md:text-5xl font-extrabold tracking-tight ${title(isDark)}`}>Your AI <span className="text-gradient">intelligence team</span></h2>
      <p className={`text-sm max-w-2xl ${muted(isDark)}`}>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {AGENTS.map((a) => (
        <div key={a.id} className={`group rounded-[2rem] border p-5 flex flex-col gap-2.5 transition-all duration-500 hover:-translate-y-1 scroll-reveal ${card(isDark)}`}>
          <div className="flex items-center gap-3">
            <span className={`w-11 h-11 rounded-2xl flex items-center justify-center glass border group-hover:scale-110 transition-transform duration-500 ${isDark ? "border-white/10" : "border-violet-500/15"} ${a.color}`}><Icon name={a.icon} className="w-5 h-5" /></span>
            <div>
...
export const UseCasesPage: React.FC<PageProps> = ({ isDark, onLaunch }) => (
  <main className="flex-1 overflow-y-auto">
    <div className="max-w-6xl mx-auto w-full p-4 md:p-8 flex flex-col gap-8">
    <header className="text-center flex flex-col items-center gap-3 scroll-reveal">
      <span className={eyebrow(isDark)}>Who it's for</span>
      <h2 className={`text-3xl md:text-5xl font-extrabold tracking-tight ${title(isDark)}`}>One platform, <span className="text-gradient">many missions</span></h2>
      <p className={`text-sm max-w-2xl ${muted(isDark)}`}>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {USE_CASES.map((u) => (
        <div key={u.persona} className={`group rounded-[2rem] border p-6 flex flex-col gap-3 transition-all duration-500 scroll-reveal ${card(isDark)}`}>
          <div className="flex items-center gap-2.5">
            <span className={`w-10 h-10 rounded-2xl glass border text-violet-500 flex items-center justify-center group-hover:scale-110 transition-transform duration-500 ${isDark ? "border-white/10" : "border-violet-500/15"}`}><Icon name={u.icon} className="w-5 h-5" /></span>
          ...
          <div className={`text-[11px] rounded-2xl border px-3 py-2 ${isDark ? "bg-black/30 border-white/5" : "bg-violet-500/[0.04] border-violet-500/10"}`}>
          ...
          <button
            ...
            className="mt-auto text-left flex items-center justify-between gap-2 bg-luna-gradient cta-luna text-white text-xs font-bold px-4 py-2.5 rounded-full"
          >
